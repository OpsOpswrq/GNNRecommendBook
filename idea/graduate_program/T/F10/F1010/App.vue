    ...
    <el-card class="box-card" style="width: 1000px;height: 500px;border-radius: 20px;left: 22%;position:fixed;top:45%;
      ...
      <div>
        ...
        <div style="float:right;width: 300px;margin-top: 10px">
          <el-button style="position: relative;top:-30px;left:70%" type="text" @click="register"><i class="el-icon-caret-right"><span style="font-size: 17px;letter-spacing: 8px">去注册</span></i></el-button>
          <el-button style="position:relative;top:-30px;" type="text" @click="admin"><i class="el-icon-caret-right"><span style="font-size: 17px;letter-spacing: 8px">管理员</span></i></el-button>
  ...
  methods:{
    ...
    onSubmit(){
      ...
      this.$refs["user"].validate((valid)=>{
        if(valid){
          $.post("/F1010/ajax1",{userId:this.user.userId.trim(" "),password:this.user.password.trim(" ")},r=>{
            res = JSON.parse(r)
            let token = res.data
            if(res.code === 200){
